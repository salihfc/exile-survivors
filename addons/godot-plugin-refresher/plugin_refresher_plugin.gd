...
func _enter_tree():
	...
	refresher.connect("request_disable_plugin", self, "_on_request_disable_plugin")
...
func _on_request_refresh_plugin(p_name):
	...
	if disabled:
		_enable_plugin(p_name)
#		refresher.show_warning(p_name)
	else:
		refresh_plugin(p_name)


func _on_request_disable_plugin(p_name):
	_disable_plugin(p_name)
...
func _disable_plugin(p_name):
	print("Disabling plugin: ", p_name)
...
func _enable_plugin(p_name):
	print("Enabling plugin: ", p_name)
